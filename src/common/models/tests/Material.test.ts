...
describe("Material", () => {
    ...
    test("fromDict parses correctly", () => {
        ...
        (expect(m.mechanical.tensileStrengthUltimate) as any).toEqualMeasurement(new Measurement(1080, "MPa"));
    });
    test("Safe tensile strength is 1/3 of max tensile strength", () => {
        const m = Material.Steel4140();
        (expect(m.getSafeMaterialStrength()) as any).toEqualMeasurement(new Measurement(990 / 3, "MPa"));
    });
