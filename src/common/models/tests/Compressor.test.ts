...
describe("Compressor", () => {
    ...
    describe.each(Compressor.getAllCompressors())("%p CFM function", (compressor) => {
        test("Returns FRC-legal values at 0 psi", () => {
            (expect((compressor as any).cfmFn(new Measurement(0, "psi"))) as any).toBeGreaterThanMeasurement(new Measurement(0, "ft^3/min"));
            (expect((compressor as any).cfmFn(new Measurement(0, "psi"))) as any).toBeLessThanMeasurement(new Measurement(1.1005, "ft^3/min"));
        });
        test("Decreases as PSI increases", () => {
            const zeroPsiCfm = (compressor as any).cfmFn(new Measurement(0, "psi"));
            const twentyPsiCfm = (compressor as any).cfmFn(new Measurement(20, "psi"));
            const fiftyPsiCfm = (compressor as any).cfmFn(new Measurement(50, "psi"));
            (expect(zeroPsiCfm) as any).toBeGreaterThanMeasurement(twentyPsiCfm);
            (expect(twentyPsiCfm) as any).toBeGreaterThanMeasurement(fiftyPsiCfm);
        });
    });
});
