...
describe("Piston", () => {
    ...
    test.each([
        ...
        (expect(p.getPushForce(pressure)) as any).toEqualMeasurement(expectedResult);
    });
    test.each([
        ...
        (expect(p.getPullForce(pressure)) as any).toEqualMeasurement(expectedResult);
    });
    ...
    test("fromDict() parses correctly", () => {
        ...
        (expect(newPiston.bore) as any).toEqualMeasurement(new Measurement(1.5, "in"));
        (expect(newPiston.rodDiameter) as any).toEqualMeasurement(new Measurement(0.375, "in"));
        (expect(newPiston.strokeLength) as any).toEqualMeasurement(new Measurement(6, "in"));
        (expect(newPiston.pullPressure) as any).toEqualMeasurement(new Measurement(50, "psi"));
        (expect(newPiston.pushPressure) as any).toEqualMeasurement(new Measurement(80, "psi"));
        (expect(newPiston.period) as any).toEqualMeasurement(new Measurement(1, "s"));
    });
});
