...
describe("Measurement", () => {
    ...
    ])("%p Simplify returns simplified jsQty", ([complexMagnitude, complexUnits], [simpleMagnitude, simpleUnits]) => {
    ...
    describe("Math functions map to inner Qty functions", () => {
        ...
        test.each([
            ...
            (expect(a.add(b)) as any).toBeCloseToMeasurement(result);
        });
        test.each([
            [
            ...
            (expect(a.sub(b)) as any).toBeCloseToMeasurement(result);
        });
        test.each([
            [
                new Measurement(5, "m"),
            ...
            (expect(a.mul(b)) as any).toBeCloseToMeasurement(result);
        });
        test.each([
            [
        ...
        test.each([
            ...
            (expect(a) as any).toBeLessThanOrEqualMeasurement(b);
        });
        test.each([
            [new Measurement(5, "in"), new Measurement(10, "in")],
            [new Measurement(10, "m"), new Measurement(10000, "cm")],
        ])("%p gt", (a, b) => {
            (expect(b) as any).toBeGreaterThanMeasurement(a);
        });
        test.each([
            [new Measurement(5, "in"), new Measurement(10, "in")],
            [new Measurement(5, "in"), new Measurement(5, "in")],
            ...
            (expect(b) as any).toBeGreaterThanOrEqualMeasurement(a);
        });
    });
    ...
    test("fromDict() parses correctly", () => {
        const d = { s: 1, u: "in" };
        (expect(Measurement.fromDict(d)) as any).toEqualMeasurement(new Measurement(1, "in"));
    });
    test("copy() returns an equal, new instance", () => {
        ...
        (expect(m1) as any).toEqualMeasurement(copy);
    });
    test.each([
        [
            new Measurement(1, "in"),
        ...
        (expect(measurement.clamp(min, max)) as any).toEqualMeasurement(expected);
    });
    test("inverse() works correctly", () => {
        (expect(new Measurement(5, "in/s").inverse()) as any).toEqualMeasurement(new Measurement(0.2, "s/in"));
        expect(() => new Measurement(0, "A/V").inverse()).toThrowError("Divide by zero");
    });
    test.each([
        ...
        (expect(measurement.abs()) as any).toEqualMeasurement(expected);
    });
    test.each([
    ...
    test.each([
        ...
        (expect(measurement.negate()) as any).toEqualMeasurement(expected);
    });
    test.each([
        ...
        (expect(measurement.removeRad()) as any).toEqualMeasurement(expected);
    });
    test("Measurement.min() works correctly", () => {
        (expect(Measurement.min(new Measurement(5, "A"), new Measurement(10, "A"))) as any).toEqualMeasurement(new Measurement(5, "A"));
    });
    test("Measurement.max() works correctly", () => {
        (expect(Measurement.max(new Measurement(5, "A"), new Measurement(10, "A"))) as any).toEqualMeasurement(new Measurement(10, "A"));
    });
});
