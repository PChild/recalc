...
describe("Motor tests", () => {
    ...
    test.each([
        ...
        expect(motor).toMatchObject({
            url: data.url,
            name: data.name,
            freeSpeed: (expect as any).toEqualMeasurement(data.freeSpeed),
            stallTorque: (expect as any).toEqualMeasurement(data.stallTorque),
            stallCurrent: (expect as any).toEqualMeasurement(data.stallCurrent),
            freeCurrent: (expect as any).toEqualMeasurement(data.freeCurrent),
            weight: (expect as any).toEqualMeasurement(data.weight),
            quantity: 1,
            kV: (expect as any).toEqualMeasurement(data.kV),
            kT: (expect as any).toEqualMeasurement(data.kT),
            maxPower: (expect as any).toBeCloseToMeasurement(data.maxPower.removeRad()),
            resistance: (expect as any).toEqualMeasurement(data.resistance),
        });
    });
});
...
const factorial = (n: any) => !(n > 1) ? 1 : factorial(n - 1) * n;
function GenerateObjectKeyCombinations(obj: any, n: any) {
    // @ts-expect-error ts-migrate(7023) FIXME: 'k_combinations' implicitly has return type 'any' ... Remove this comment to see the full error message
    function k_combinations(set: any, k: any) {
...
test.each([
    ...
    expect(combos).toHaveLength(factorial(props) / (factorial(n) * factorial(props - n)));
});
const motor = {
...
// @ts-expect-error ts-migrate(2345) FIXME: Argument of type '(incompleteState: any) => void' ... Remove this comment to see the full error message
describe.each(combos)("%p MotorState combinations", (incompleteState) => {
    test("Terminates and completes state with high current limit", () => {
        const ms = new MotorState(Motor.Falcon500s(1), new Measurement(500, "A"), incompleteState).solve();
        expect(ms.solved).toBeTruthy();
        expect(ms).toMatchObject({
            voltage: (expect as any).toBeCloseToMeasurement(motor.voltage, 1),
            rpm: (expect as any).toBeCloseToMeasurement(motor.rpm, -2),
            current: (expect as any).toBeCloseToMeasurement(motor.current, -1),
            torque: (expect as any).toBeCloseToMeasurement(motor.torque, 1),
            power: (expect as any).toBeCloseToMeasurement(motor.power, -2),
        });
    });
    test("Terminates and completes state with moderate current limit", () => {
        const currentLimit = new Measurement(40, "A");
        const ms = new MotorState(Motor.Falcon500s(1), currentLimit, incompleteState).solve();
        expect(ms.solved).toBeTruthy();
        expect(ms.didLimitCurrent).toBeTruthy();
        (expect(ms.voltage) as any).toBeGreaterThanMeasurement(new Measurement(0, "V"));
        (expect(ms.voltage) as any).toBeLessThanOrEqualMeasurement(new Measurement(12, "V"));
        (expect(ms.current) as any).toBeGreaterThanMeasurement(new Measurement(0, "A"));
        (expect(ms.current) as any).toBeLessThanOrEqualMeasurement(currentLimit);
    });
});
