...
import { QueryableParamHolder, queryStringToDefaults, stateToQueryString, } from "../query-strings";
describe("Query strings", () => {
    describe("queryStringToDefaults", () => {
        test.each([
            ...
            {
                query: '?pitch=%7B"s"%3A3%2C"u"%3A"mm"%7D&foo=%7B"s"%3A25%2C"u"%3A"kg"%7D',
                queryParams: {
                ...
                expected: {
                    pitch: (expect as any).toEqualMeasurement(new Measurement(3, "mm")),
                    foo: (expect as any).toEqualMeasurement(new Measurement(25, "kg")),
                },
            },
        ])("%p Works with no conversion function and everything provided", ({ query, queryParams, defaults, expected }) => {
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 4 arguments, but got 3.
            expect(queryStringToDefaults(query, queryParams, defaults)).toMatchObject(expected);
        });
        test.each([
            {
                query: "?a=1&b=foo&c=0",
            ...
            },
            {
                ...
                expected: {
                    pitch: (expect as any).toEqualMeasurement(new Measurement(3, "mm")),
                    foo: (expect as any).toEqualMeasurement(new Measurement(25, "kg")),
                    bar: (expect as any).toEqualMeasurement(new Measurement(12, "N")),
                },
            },
        ])("%p Works with no conversion function and some things missing", ({ query, queryParams, defaults, expected }) => {
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 4 arguments, but got 3.
            expect(queryStringToDefaults(query, queryParams, defaults)).toMatchObject(expected);
        });
        test("Works with no conversion function and extra params provided", () => {
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 4 arguments, but got 3.
            const result = queryStringToDefaults('?pitch=%7B"s"%3A3%2C"u"%3A"mm"%7D&foo=%7B"s"%3A25%2C"u"%3A"kg"%7D', {
                pitch: Measurement.getParam(),
            }, {
        ...
        test.each([
            {
                qphs: [
                    ...
                    new QueryableParamHolder({ baz: new Measurement(5, "ft") }, Measurement.getParam()),
                ],
                expected: "bar=1&baz=%7B%22s%22%3A60%2C%22u%22%3A%22in%22%7D&foo=2",
            },
            {
                qphs: [
                    new QueryableParamHolder({ foo: new Measurement(5, "N * m / s^2") }, Measurement.getParam()),
