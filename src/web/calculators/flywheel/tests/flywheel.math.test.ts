...
describe("Flywheel math", () => {
    ...
    ])("%p Calculate windup time", ({ momentOfInertia, motorFreeSpeed, motorStallTorque, motorQuantity, ratio, targetSpeed, expected, }) => {
        (expect(calculateWindupTime(momentOfInertia, motorFreeSpeed, motorStallTorque, motorQuantity, ratio, targetSpeed).toBase()) as any).toBeCloseToMeasurement(expected);
    });
    test.each([
        {
            momentOfInertia: new Measurement(20, "lb in^2"),
    ...
    ])("%p Generate Chart data", ({ momentOfInertia, motorFreeSpeed, motorStallTorque, motorQuantity, currentRatio, targetSpeed, length, first, last, }) => {
        const data = generateChartData(momentOfInertia, motorFreeSpeed, motorStallTorque, motorQuantity, currentRatio, targetSpeed);
